// 参考：https://github.com/ReneNyffenegger/cpp-base64/blob/master/base64.cpp
...
string base64_decode(const string& encoded_string) {
    ...
    while (in_len-- && (encoded_string[len] != '=')) {
        unsigned char chr = encoded_string[len];
        if (!is_base64(encoded_string[len])) {
            if (chr == '\n' || chr == ' ') {
                len++;
            } else if (chr == '\r' && encoded_string[len + 1] == '\n') {
                len += 2;
            } else {
                break;
            }
